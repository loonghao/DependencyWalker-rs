...
impl DllResolver {
    ...
    pub fn add_search_path<P: AsRef<Path>>(&mut self, path: P) {
        self.config.custom_search_paths.push(path.as_ref().to_path_buf());
    }
    ...
    pub fn resolve_dll(&mut self, root_pe: &PEFile, dll_name: &str) -> Result<(ModuleSearchStrategy, Option<PathBuf>)> {
        ...
        log::debug!("Resolving DLL: {} (normalized: {})", dll_name, normalized_name);
        ...
        log::debug!("Target architecture: {}, WOW64: {}", processor_arch, is_wow64);
        ...
        if self.config.enable_api_set_schema {
            if let Some(redirected_name) = self.resolve_api_set(&normalized_name)? {
                log::debug!("API Set redirection: {} -> {}", normalized_name, redirected_name);
                // Search for the redirected DLL using the standard search order
                let redirected_path = self.search_for_dll(&redirected_name, is_wow64, &processor_arch)?;
        ...
        if let Some(app_dir) = self.get_application_directory(root_pe) {
            if let Some(path) = self.search_in_directory(&app_dir, &normalized_name, &processor_arch)? {
        ...
        for system_dir in &system_dirs {
            if let Some(path) = self.search_in_directory(system_dir, &normalized_name, &processor_arch)? {
        ...
        if let Some(path) = self.search_in_directory(&windows_dir, &normalized_name, &processor_arch)? {
        ...
        if let Some(working_dir) = self.get_working_directory() {
            if let Some(path) = self.search_in_directory(&working_dir, &normalized_name, &processor_arch)? {
        ...
        for custom_path in &self.config.custom_search_paths {
            if let Some(path) = self.search_in_directory(custom_path, &normalized_name, &processor_arch)? {
        ...
        if Path::new(&normalized_name).is_absolute() && Path::new(&normalized_name).exists() {
            ...
            return Ok((ModuleSearchStrategy::Fullpath, Some(PathBuf::from(normalized_name))));
        }
...
impl DllResolver {
    ...
    fn search_in_directory(&self, dir: &Path, dll_name: &str, processor_arch: &str) -> Result<Option<PathBuf>> {
            ...
            if let Ok(pe_map) = crate::core::pe_parser::PEFileMap::new(&dll_path) {
                if let Ok(pe_file) = crate::core::pe_parser::PEFile::new(&pe_map) {
                    ...
                    if target_arch == processor_arch || processor_arch == "unknown" {
                        ...
                        log::debug!("Architecture mismatch: {} (expected: {}, found: {})",
                                  dll_path.display(), processor_arch, target_arch);
                    }
                } else {
    ...
    fn search_in_path(&self, dll_name: &str, processor_arch: &str) -> Result<Option<PathBuf>> {
        if let Ok(path_var) = env::var("PATH") {
            for path_dir in path_var.split(';') {
                ...
                if let Some(found_path) = self.search_in_directory(&dir_path, dll_name, processor_arch)? {
    ...
    fn load_known_dlls(&self, is_wow64: bool) -> Result<HashMap<String, PathBuf>> {
        ...
        let system_dir = if is_wow64 {
            self.get_syswow64_directory().unwrap_or_else(|| self.get_system32_directory().unwrap())
        } else {
        ...
        log::debug!("Loaded {} KnownDLLs for {} architecture",
                   known_dlls.len(),
    ...
    fn resolve_api_set(&mut self, dll_name: &str) -> Result<Option<String>> {
        ...
        if let Some(ref cache) = self.api_set_cache {
            if let Some(redirected_name) = cache.get(&dll_name.to_lowercase()) {
                log::debug!("API Set redirection found: {} -> {}", dll_name, redirected_name);
    ...
    fn load_api_set_schema(&self) -> Result<HashMap<String, String>> {
        ...
        let common_api_sets = [
            ...
            ("api-ms-win-core-processenvironment-l1-1-0.dll", "kernel32.dll"),
    ...
    fn search_for_dll(&mut self, dll_name: &str, is_wow64: bool, processor_arch: &str) -> Result<Option<PathBuf>> {
        ...
        for system_dir in &system_dirs {
            if let Some(path) = self.search_in_directory(system_dir, dll_name, processor_arch)? {
                log::debug!("Found redirected DLL in system directory: {}", path.display());
                return Ok(Some(path));
            }
        }
        ...
        if let Some(path) = self.search_in_directory(&windows_dir, dll_name, processor_arch)? {
            log::debug!("Found redirected DLL in Windows directory: {}", path.display());
            return Ok(Some(path));
        }
        ...
        for custom_path in &self.config.custom_search_paths {
            if let Some(path) = self.search_in_directory(custom_path, dll_name, processor_arch)? {
                log::debug!("Found redirected DLL in custom search path: {}", path.display());
                return Ok(Some(path));
            }
        }
    ...
    fn test_api_set_integration() {
        ...
        for api_dll in &api_set_dlls {
            ...
            if let Ok(Some(redirected)) = result {
                assert!(!redirected.is_empty(), "Redirected DLL name should not be empty for {}", api_dll);
                assert!(!redirected.starts_with("api-ms-win-"), "Redirected DLL should not be another API Set DLL: {} -> {}", api_dll, redirected);
