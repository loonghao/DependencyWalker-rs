...
use dependencywalker_rs::core::tree::{TraversalOrder, NodeFilter};
use std::env;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(root) = &tree.root {
        ...
        println!("  Architecture: {}", 
            root.is_64bit.map_or("Unknown".to_string(), |is_64| 
                if is_64 { "x64".to_string() } else { "x86".to_string() }
            )
        );
    ...
    println!("\nAll Unique Dependencies ({}):", tree.all_dependencies.len());
    for (i, (path, info)) in tree.all_dependencies.iter().enumerate() {
        ...
        println!("  {}. {} {} [{}] (refs: {}, depth: {})", 
            i + 1, status, path.display(), arch, info.reference_count, info.first_seen_depth);
    }
    
    // Display missing dependencies
    if !tree.missing_dependencies.is_empty() {
        println!("\nMissing Dependencies ({}):", tree.missing_dependencies.len());
    ...
    // Missing nodes only
    let missing_nodes: Vec<_> = tree.iter_filtered(
        TraversalOrder::PreOrder,
        NodeFilter::missing_only()
    ).collect();
...
fn print_tree_node(node: &dependencywalker_rs::core::dependency::DependencyNode, level: usize, max_level: usize) {
    if level > max_level {
        return;
    }
    ...
    for (i, child) in node.children.iter().enumerate() {
        if i < 5 { // Limit to first 5 children for readability
            print_tree_node(child, level + 1, max_level);
        } else if i == 5 {
            let child_indent = "  ".repeat(level + 1);
            println!("{}... and {} more children", child_indent, node.children.len() - 5);
            break;
        }
    }
}
