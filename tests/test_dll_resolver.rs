...
fn test_simple_resolution(resolver: &mut DllResolver) -> Result<(), Box<dyn std::error::Error>> {
    let test_dlls = ["kernel32.dll", "user32.dll", "ntdll.dll", "nonexistent.dll"];
...
fn test_pe_context_resolution(
    resolver: &mut DllResolver,
...
fn test_custom_search_paths(resolver: &mut DllResolver) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match resolver.resolve_dll_simple("kernel32.dll") {
        Ok(Some(path)) => {
            println!(
                "  ✓ Found kernel32.dll with custom paths: {}",
                path.display()
            );
        }
        Ok(None) => {
...
fn test_different_configurations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match resolver2.resolve_dll_simple("kernel32.dll") {
        Ok(Some(path)) => {
            println!(
                "    ✓ Found kernel32.dll without KnownDLLs: {}",
                path.display()
            );
        }
        Ok(None) => {
    ...
    println!(
        "    Working directory: {:?}",
        resolver3.config().working_directory
    );
